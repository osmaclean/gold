...
export default async function AdminDashboardPage() {
  const [playerCount, matchCount, seasonActive, recentMatches, lastSnapshot] = await Promise.all([
    ...
    db.cardSnapshot.findFirst({ orderBy: { createdAt: "desc" }, include: { card: { include: { user: true } } } }),
  ])

  return (
    <div className="flex flex-col gap-8">
      <div>
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-gold-300">
          <Shield className="size-3.5" /> Painel Admin
        </div>
        <h1 className="font-display text-4xl md:text-5xl tracking-widest mt-2 text-shine">
          CONTROLE DA TROPA
        </h1>
        <p className="text-muted-foreground mt-2">Gerencie partidas, jogadores, cartas e temporadas.</p>
      </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard label="Season ativa" value={seasonActive ? `#${seasonActive.number}` : "—"} icon={Trophy} accent="green" />
        <KpiCard
          label="Último update"
        ...
        <Card className="p-6">
          ...
          {recentMatches.length === 0 ? (
            ...
            <ul className="flex flex-col gap-2">
              {recentMatches.map((m) => (
                <li key={m.id} className="flex items-center justify-between p-3 rounded-md bg-white/[0.03]">
                  <div>
                    <p className="text-sm font-medium">{m.map.replace("de_", "").toUpperCase()}</p>
                    <p className="text-[10px] text-muted-foreground">{formatDateTimeBR(m.playedAt)}</p>
                  </div>
                  <div className="font-display text-xl numeric">
...
function AdminQuickLink({ href, icon: Icon, label }: { href: string; icon: typeof Users; label: string }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 p-4 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 hover:border-gold-500/30 transition-colors"
    >
      <span className="size-9 grid place-items-center rounded-md bg-gold-500/10 text-gold-300">
